var names = [
  'Object', 'String', 'Boolean', 'Number', 'RegExp', 'Date', 'Array', 'Function',
  'Symbol', 'Map', 'Set', 'WeakMap', 'Error', 'Promise'
]
...
Primitives.prototype.replace = function(value){
  ...
  if (~primIndex){
    let name = names[primIndex]
    return this.context[name]
  } else if (~protoIndex) {
    let name = names[protoIndex]
...
Primitives.prototype.applyNew = function(func, args){
  if (func.wrapped){
    ...
    var instance = new prim(...args) // (Function.prototype.bind.apply(prim, arguments))
