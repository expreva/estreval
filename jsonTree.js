    ...
    forEachNode : function(obj, func) {
      ...
      case 'object':
        ...
        keys.forEach(function(item, i) {
          ...
          // CHANGED: Skip location meta from each node

          if (['start', 'end', 'loc', 'range'].indexOf(item) >= 0) return
    ...
    var self = this,
      ...
      template = function(label, val) {
        var str = `<span class="jsontree_label-wrapper">
          <span class="jsontree_label">${label}</span>: </span>
        ...
        var comma = (!isLast) ? ',' : '',
          str = `<div class="jsontree_value-wrapper">
            <div class="jsontree_value jsontree_value_${self.type}">
              <b>${sym[0]}</b><span class="jsontree_show-more">&hellip;</span><ul class="jsontree_child-nodes"></ul><b>${sym[1]}</b></div>${comma}
          </div>`
